...
def generate_graph():
    ...
    graph_name_gen = graph_name_entry.get().replace(" ", "_")
    ...
    if no_fields_empty(): #if the path is complete
        if os.path.exists(check_dir): #dir exists
            if not os.listdir(check_dir):  #dir is empty
                call_cookiecutter(graph_root_gen, True) #flag that directory exists
            else: #if the directory is not empyy
                warning_graph_exists()
        else: #directory does not exist
            call_cookiecutter(check_dir, graph_root_gen, False) #flag that directory does not exist



def call_cookiecutter(full_path, root_path, dir_exists=False):
    if not dir_exists:
        #print("Creating Directory...")
        #os.mkdir(graph_dir) #only make directory as a test here
        print(" ") #pass
    print("GENERATING Luigi graph...")
    print("Installing into: ", full_path)
    # call cookiecutter
    cookiecutter_command = ('pipenv run cookiecutter https://github.com/mjnickerson/cookiecutter-luiGUI --output-dir "' + root_path + '" --no-input')
    print(cookiecutter_command)
    os.system(cookiecutter_command)
    # end
    print("Done!")
...
def delete_graph():
    ...
    if os.path.exists(check_dir):
        ...
        shutil.rmtree(check_dir)  # delete all test temp files
    else:
        warning_no_folder()
